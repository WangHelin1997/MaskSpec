...
def get_mean_std(n_mel=128, sample_number=10000):
    print('Start...')
    hdf5_file = './dcase18/data/dcase18_train_mp3.hdf'
    ...
    np.save('./dcase18/mean_std_'+str(n_mel)+'.npy', dict)
    return {"done": True}

def test(n_mel=128):
    mean_std_file = np.load('./dcase18/mean_std_'+str(n_mel)+'.npy',allow_pickle=True).item()
