...
if os.path.exists('./openmic18/data') == False:
    os.mkdir('./openmic18/data')
dataset_path = "./openmic18/data/"
mp3_path = os.path.join(dataset_path, "mp3/")
hdf5s_dir = "./openmic18/data/"
train_files_csv = os.path.join(dataset_path, extract_target, "openmic-2018/partitions/split01_train.csv")
test_files_csv = os.path.join(dataset_path, extract_target, "openmic-2018/partitions/split01_test.csv")


def download(force=False):
    if force or not os.path.isfile(dataset_path + download_target):
        print("Downloading OpenMIC from zenodo...")
        download_url_to_file(openmicurl, dataset_path + download_target)
    else:
        print(f"{download_target} already exists. Skipping download!")


def untar():
    my_tar = tarfile.open(dataset_path + download_target)
    print(f"Extracting openmic from {download_target} to {extract_target}")

    my_tar.extractall(dataset_path + extract_target)


def process_folder(fol="openmic2018"):
    print("now working on ", fol)
    os.makedirs(mp3_path + fol, exist_ok=True)
    all_files = list(glob.glob(os.path.join(dataset_path, extract_target, "openmic-2018/audio/") + "/*/*.ogg"))  # openmic format
...
def pack():
    d_files = dict()
    opmic = np.load(os.path.join(dataset_path, extract_target, "openmic-2018/openmic-2018.npz"))
    ...
    for read_file, prefix in [(train_files_csv, "openmic2018"), (test_files_csv, "openmic2018")]:
