...
from openmic18.engine_run import train_one_epoch, evaluate
from openmic18.dataset import get_training_set, get_test_set


def get_args_parser():
    parser = argparse.ArgumentParser('openmic18 classification', add_help=False)
    ...
    # Dataset parameters
    parser.add_argument('--data_path_train', default='./openmic18/data/mp3/openmic_train.csv_mp3.hdf', type=str,
                        help='train dataset path')
    parser.add_argument('--data_path_test', default='./openmic18/data/mp3/openmic_test.csv_mp3.hdf', type=str,
                        help='test dataset path')
    parser.add_argument('--norm_file', default='./openmic18/mean_std_128.npy', type=str,
                        help='norm file path')
    ...
    parser.add_argument('--specmix', default=False, type=bool)
    ...
    parser.add_argument('--output_dir', default='./openmic18/output_dir',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='../openmic18/log_dir',
...
def main(args):
    ...
    if args.eval:
        ...
        print(f"Accuracy of the network on the {len(dataset_test)} test images: {test_stats['mAP']:.1f}%")
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = evaluate(data_loader_test, model, device)
        print(f"Accuracy of the network on the {len(dataset_test)} test images: {test_stats['mAP']:.3f}")
        max_accuracy_test = max(max_accuracy_test, test_stats["mAP"])
        print(f'Max accuracy Test: {max_accuracy_test:.3f}')
